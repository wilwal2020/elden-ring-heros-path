...
one long green line, and you can watch it play back from the beginning. The
Lands Between has no such thing. This makes one: it reads your position out of
the running game, keeps it, and draws every step you have taken on a map you
can scroll, with the whole history as one continuous path rather than a pile of
per-session files.

It goes a little further than Zelda's does, because Elden Ring needs it to.
Caves and catacombs are their own maps rather than a scribble on the overworld.
Teleports break the line instead of drawing a walk you never took. The
underground rivers are a second map you drop into. Deaths are marked, with the
grace you got up at. And the playback runs the whole thing back at up to an
hour a second, easing off at each teleport so you can see where you went.
...
Built to fix four specific things about the Nexus mod it replaces
([Route Tracker, #9294](https://www.nexusmods.com/eldenring/mods/9294)), which
broke on patch 1.17:

| Problem | Fix |
|---|---|
| Every session made a separate file, so seeing your full path meant opening all of them | One SQLite database. Sessions are a column; the viewer shows everything by default |
| Map loaded slowly on every scroll | Real tile pyramid, canvas rendering, zoom-aware path simplification |
| Caves looked like overworld wandering | Interiors are separate maps and never touch the world plane; they become markers instead |
| Teleports drew a line as if you'd walked there | Line breaks on map change or implausible speed |

It also survives game patches, which is the thing that killed the last one. See
